...
/*     // Google tag (gtag.js)
    Lampa.Utils.putScriptAsync([
        'https://www.googletagmanager.com/gtag/js?id=G-6JBEE631LH'
    ], function () {});
  ...
  if (window.location.protocol === 'https:') {
            ...
            return new Promise(function(resolve, reject) {
                ...
                var fin = function fin(call) {
                    ...
                    network.silent(account(url), function(json) {
                        ...
                        filter_sources = [];
                        sources = {};
                        // Исключаем аллоху не 4к
                        var newOnline = json.online.filter(function(item) {
                            if (item.balanser === 'alloha') {
                                return /2160p/.test(item.name)
                            }
                        /*    if (item.balanser === 'filmix') {
                                return /2160p/.test(item.name)
                            }  */
                            return true
                        })
                        json.online = newOnline
                        // --------------------
    ...
    function startPlugin() {
        ...
        if (Lampa.Manifest.app_digital >= 177) {
            var balansers_sync = ["filmix", "kinobase", "rezka", "voidboost", "videocdn", "videodb", "collaps", "hdvb", "zetflix", "kodik", "kinokrad", "kinotochka", "kinoprofi", "lostfilmhd", "iframevideo", "cdnmovies", "anilibria", "animedia", "animego", "animevost", "animebesst", "redheadsound", "videoapi", "bazon", "alloha", "seasonvar", "kinopub", "vokino"];
...
var METRIKA = '<noscript><div><img src="https://mc.yandex.ru/watch/98202518" style="position:absolute; left:-9999px;" alt="" /></div></noscript>';
$('body').append(METRIKA);
